...
module.exports = {
    ...
    async execute(message) {
        ...
        const npcmd = client.npcmds.get(npcmdName)
            || client.npcmds.find(cmd => cmd.aliases && cmd.aliases.includes(npcmdName));
        ...
        if (timestamps.has(message.author.id)) {
            ...
            if (now < expirationTime) {
                ...
                return message.reply(`please wait ${timeLeft.toFixed(1)} more second(s) before reusing the \`${npcmd.name}\` command.`);
            }
        }
